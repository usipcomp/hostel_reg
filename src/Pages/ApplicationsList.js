...
const ApplicationsList = ({ showAlert }) => {
    ...
    const getApplications = async () => {
      try {
        ...
        const array = Array.from(res.data).filter((app) => {
          return (
            app.allotedStatus !== "rejected" && app.allotedStatus !== "accepted"
          );
        });
  ...
  const remove_app = async (id) => {
    const response = await fetch(
      `http://localhost:4000/hostelreg/applications/auth/application/${id}`,
      {
        method: "PUT",
        headers: {
  ...
  const runAlgo = async (event) => {
    ...
    let fromdate =
      event.target[0].id === "from_date" ? event.target[0].value : "01/01/2023";
    let todate =
      event.target[1].id === "to_date" ? event.target[1].value : "01/01/2023";
    let admn_year =
      event.target[2].id === "admn_year" ? event.target[2].value : "2019";
    let current_year =
      event.target[3].id === "current_year" ? event.target[3].value : 1;
  ...
  const addintoExcel = (array) => {
    array.forEach((element) => {
      excelCSV.push([
        ...
        element.semester,
        element.back_papers,
        element.year_of_admission,
        element.distance,
        element.blood_group,
        element.region,
        element.sgpa,
        element.chronic_problems,
        element.preferences,
        element.father_name,
        element.father_phone_no,
        element.father_email,
        element.father_occupation,
        element.father_office_address,
        element.father_office_no,
        element.mother_name,
        element.mother_phone_no,
        element.mother_email,
        element.mother_occupation,
        element.mother_office_address,
        element.mother_office_phone_no,
        element.acc_no,
        element.acc_holder_name,
        element.bank_name,
        element.ifsc,
        element.bank_branch,
        ...
        element.home_city,
        element.home_state,
        element.home_country,
        element.home_pincode,
        element.corr_address,
        element.corr_city,
        element.corr_state,
        element.corr_country,
        element.corr_pincode,
        element.local_guardian_name,
        element.local_guardian_address,
        element.local_guardian_phone_no,
        element.local_guardian_email,
        element.sign,
        element.discrepancy,
        element.ProfilePic,
        element.allotedStatus,
      ]);
    });
    return excelCSV;
  };
  const renderedApplications = applications.map((app, ind) => {
    return (
      <div
        key={ind}
        className="w-full h-fit bg-gray-200 p-4 rounded-xl my-4 flex justify-between shadow-lg hover:scale-105 duration-500 hover:bg-gray-300 cursor-pointer"
      >
        ...
        <div className="flex">
          <Link
            to={`/${app._id}`}
            className="text-white self-center cursor-pointer bg-[#006d77] p-2 border-none hover:scale-105 duration-500 h-10 mx-auto flex"
          >
            <MdOutlinePictureAsPdf className="mr-2"></MdOutlinePictureAsPdf>
            <div>PDF</div>
          </Link>
          <Button
            handleClick={() => {
    ...
    <div className="h-fit min-h-screen bg-[#edf6f9] w-full">
      {applications.length ? (
        <CSVLink
          ...
          className="btn"
          data={addintoExcel(applications)}
          target="_blank"
        >
          Download Excel/Csv
        </CSVLink>
